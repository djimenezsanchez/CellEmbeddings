...
flags.DEFINE_string('train_prefix', '../Graph_1/', 'name of the object file that stores the training data. must be specified.')

# left to default values in main experiments 
flags.DEFINE_integer('epochs', 1, 'number of epochs to train.')
flags.DEFINE_float('dropout', 0.0, 'dropout rate (1 - keep probability).')
flags.DEFINE_float('weight_decay', 0.0, 'weight for l2 loss on embedding matrix.')
flags.DEFINE_integer('max_degree', 67, 'maximum node degree.')
flags.DEFINE_integer('samples_1', 67, 'number of samples in layer 1')
flags.DEFINE_integer('samples_2', 67, 'number of users samples in layer 2')
flags.DEFINE_integer('samples_3', 67, 'number of users samples in layer 3')
flags.DEFINE_integer('samples_4', 67, 'number of users samples in layer 4')
...
flags.DEFINE_integer('dim_3', 64, 'Size of output dim (final is 2x this, if using concat)')
flags.DEFINE_integer('dim_4', 64, 'Size of output dim (final is 2x this, if using concat)')
flags.DEFINE_boolean('random_context', False, 'Whether to use random context or direct edges')
flags.DEFINE_integer('neg_sample_size', 20, 'number of negative samples')
flags.DEFINE_integer('batch_size', 10000, 'minibatch size.')
flags.DEFINE_integer('n2v_test_epochs', 0, 'Number of new SGD epochs for n2v.')
flags.DEFINE_integer('identity_dim', 0, 'Set to positive value to use identity embedding features of that dimension. Default 0.')
flags.DEFINE_integer('context_area', 2500, 'Contextual area to cover per structure(in pixels)')
flags.DEFINE_integer('hops', 3, 'Contextual area to cover per structure(in pixels)')


#logging, saving, validation settings etc.
flags.DEFINE_boolean('save_embeddings', True, 'whether to save embeddings for all nodes after training')
flags.DEFINE_string('base_log_dir', '', 'base directory for logging and saving embeddings')
flags.DEFINE_integer('validate_iter', 10000000000, "how often to run a validation minibatch.")
flags.DEFINE_integer('validate_batch_size', 0, "how many nodes per validation sample.")
flags.DEFINE_integer('gpu',0, "which gpu to use.")
flags.DEFINE_integer('print_every', 1, "How often to print training info.")
flags.DEFINE_integer('max_total_steps', 10**9, "Maximum total number of iterations")
flags.DEFINE_boolean('new_version', False, "Include all changes done by Daniel Jimenez")
...
def save_val_embeddings(sess, model, minibatch_iter, size, out_dir, mod=""):
    ...
    np.save(out_dir + 'nodes' + mod + ".npy", nodes)
...
def train(train_data, test_data=None):
    ...
    minibatch = EdgeMinibatchIterator(G, 
            ...
            max_degree=FLAGS.max_degree, sizes=sizes, context_area=FLAGS.context_area,
    ...
    if FLAGS.model == 'graphsage_mean':
        # Create model
        sampler = UniformNeighborSampler(adj_info) # What????
        if FLAGS.hops==1:
            ...
            layer_infos = [SAGEInfo("node", sampler, FLAGS.samples_1, FLAGS.dim_1),
                                SAGEInfo("node", sampler, FLAGS.samples_2, FLAGS.dim_2),
                                SAGEInfo("node", sampler, FLAGS.samples_3, FLAGS.dim_3),
                                SAGEInfo("node", sampler, FLAGS.samples_4, FLAGS.dim_4)]
    ...
    for epoch in range(FLAGS.epochs): 
        ...
        while not minibatch.end(): #_10 
            ...
            if shadow_mrr is None:
                ...
                shadow_mrr -= (1-0.99) * (shadow_mrr - 0)
    ...
    if FLAGS.save_embeddings:
        ...
        save_val_embeddings(sess, model, minibatch, FLAGS.batch_size, log_dir())
...
def main(argv=None):
    ...
    train_data = load_data(FLAGS.train_prefix, normalize=True, load_walks=False)
